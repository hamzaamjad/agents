...
> Origin: copied 2026-06-10 from `hamzaamjad/tickets` (github.com/hamzaamjad/tickets),
> path `docs/runbooks/worktree-recovery.md`, commit `c0296f9` ("FEAT-004: add worktree
> recovery runbook"). Bundled here so the skill is self-contained on any deployment
> host. Two pointers were adapted to resolve from this location; content is otherwise
> verbatim.
...
in [../SKILL.md](../SKILL.md) (this skill); this runbook is what
...
Sources: `docs/research/r5-worktree-failure-recovery.md` in the origin
repo (`hamzaamjad/tickets`). Every procedure here follows the same arc:
**diagnose → stabilize → repair → verify.** Destructive steps are
flagged explicitly.
